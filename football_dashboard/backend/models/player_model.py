...
def get_player_by_registration(registration_number):
    ...
    try:
        ...
        cursor.execute("SELECT * FROM players WHERE registration_number = %s", (registration_number,))
        player = cursor.fetchone()
...
def update_player(player_id, data):
    ...
    try:
        ...
        allowed_fields = ['name', 'dob', 'nationality', 'jersey_number', 
                          'position', 'photo_url', 'school_id', 'academy_id', 'club_id']
